...
def viewColumnImage( column, orgrows, orgcols, name ):
    """
    Reshape and normalise a 1-D column vector back into a displayable image.

    Args:
        column:   Flat 1-D NumPy array (eigenvector, difference image, or original image).
        orgrows:  Number of rows in the target image.
        orgcols:  Number of columns in the target image.
        name:     Window name passed to cv2.imshow.
    """
...
def main(argv):
    """
    Build an eigenspace from all images in a directory, display the top 10
    eigenvectors, project a selection of images into the 6-dimensional eigenspace,
    and print pairwise Euclidean distances between their embeddings.

    Args:
        argv:  Command-line arguments. argv[1] must be the path to the image directory.
    """
    # all images must be the same size; the first image sets the dimensions
    orgrows = 0
    orgcols = 0
